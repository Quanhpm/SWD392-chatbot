# EduSmart — Software Requirements Specification (SRS)
...
EduSmart là hệ thống quản lý tài liệu học tập và hỏi đáp thông minh bằng **RAG (Retrieval-Augmented Generation)** dành cho môi trường đại học. Hệ thống tổ chức nội dung trực tiếp theo **môn học (Subject)**, không chia theo lớp học.

- **Admin** quản lý tài khoản, môn học và phân công giảng viên.
- **Teacher** upload và quản lý tài liệu của mình trong các Subject đang được phân công.
- **Student** đã đăng nhập được xem toàn bộ Subject active, tài liệu sẵn sàng và hỏi AI theo tài liệu được chọn.

### 1.1. Mục tiêu chính

1. Quản lý tài khoản theo ba vai trò: Admin, Teacher và Student.
2. Quản lý Subject và quan hệ phân công nhiều-nhiều giữa Subject với Teacher.
3. Cho phép Teacher tự công bố tài liệu sau khi hệ thống xử lý thành công.
4. Cho phép Student truy cập toàn bộ nội dung học tập mà không cần mã tham gia hoặc mật khẩu môn học.
5. Trả lời câu hỏi bằng RAG theo đúng một tài liệu.
6. Quản lý quota câu hỏi theo User trong từng tháng UTC.
7. Gửi email cho các sự kiện tài khoản và phân công Subject.
...
3. Hệ thống phải kiểm tra phần mở rộng, MIME type và chữ ký thực tế (magic bytes) của file.
...
- Chat Session phải tránh vượt giới hạn document size của MongoDB (16 MB).
...
## 7. Tiêu chí hoàn thành phiên bản Subject-only
